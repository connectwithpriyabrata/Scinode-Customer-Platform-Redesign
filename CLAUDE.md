...
- **Files**: `assets/ua/` — `ua.css` (all `ua-`/`ub` styles), `ua-core.js` (seeds, state, `UA.access` effective-permission rule, `UA.capacity` Free/Premium checks, modal/drawer/menu/toast primitives), `ua-views.js` (5 tab renderers), `ua-flows.js` (member/team drawers, Add New User, Create Team, upgrade popups + intent capture, approvals, Superadmin transfer, in-module Org Activity preview), `ua-app.js` (bootstrap + presenter bar), `bp.css` + `bp.js` (blueprint page: scope map, role matrix, `canUserAccess` 14-step simulator, approval resolver comparing PRD vs Design Spec, 16 doc conflicts, phase plan).
