...
## Opportunities for Your Business — Recommendation section (Variation D: Live Signal Split)

Implemented per `Recommendation Section V1` PRD + `OPPORTUNITY_INTERACTION_SPEC.md` + approved screenshots. Adapted from the React/Tailwind/framer spec to our vanilla single-file stack and navy/teal + Poppins/Outfit system (no monospace).

- **Placement:** after "Your Activity" in **Day 0, Day 1, Day 10** (reversible `.opps` blocks, `OPPS:start/end` markers).
- **Progression:** driven by the existing day toggle (Day 0 → Level 1 Profile · Day 1 → Level 3 Request · Day 10 → Level 4 Project); **Activity Started** = Level 2 sub-toggle (Profile / Activity started) on Day 0.
- **Layout:** category filter tabs (All/Product/Market/Technology/Distribution) + search → **Live Signal Split**: dark immersive feed card (category badge, source, LIVE pulse, star/dismiss, title/subtitle/desc, 3-metric grid, Impact, "Explore Technical Dossier") + scrollable candidate stack (relevance bar, category, arrow, "scroll for more" cue when >3).
- **Drawer (shared `#opps-drawer`):** category dossier tag, relevance/impact stats, mini-markdown report, per-category **Action Center** (Product→sample dispatch, Market→RFQ, Technology→feasibility study, Distribution→route planner) → confirmation; Save/PDF/Share footer; Escape/backdrop close.
- **State (per instance):** selectedCategory, highlightedId, search, favorites, dismissed; mock data per state. **Motion:** LIVE ping, content fade-up, candidate select transitions, arrow translate, scroll-fade + pulse cue, drawer slide-in.
- Verified: 3 sections render, progression + sub-state, select/filter/search/star/dismiss, drawer + action confirmation, no console errors.

**V1 design refinement (`design refinement.pdf`, source of truth for the section UI):** applied to the existing implementation — no redesign, interaction model / drawer / states / IA preserved.
- **Right signal cards — full redesign for content density & reduced noise.** Removed the colored left bar (`::before`), the relevance progress bar, the per-card arrow icon, and the heavy accent styling. New neutral card: white surface, 1px `--border`, 16px radius, hover = `translateY(-2px)` + soft shadow (≤.22s motion). New info hierarchy = **category pill (small dot + label) + relevance score top-right (`NN% Match`, teal bold number)** → **2-line title** → **1-line summary** (`it.subt`). Active/selected card: slightly darker surface (`#fbfdfc`), soft teal glow, short 2px category accent line — no full colored border. Category dots per spec (Product→blue `#2563EB`, Market→green `#16A34A`, Technology→teal `#02968A`, Distribution→orange `#EA580C`) via new `CAT[x].dot`; the dark feed card keeps its existing category pill colors.
- **Featured card — compact spacing.** `justify-content:space-between` → `flex-start` with spec gaps (header→title 20, title→sub 6, sub→desc 8, desc→metrics 24, metrics→foot 20); removes the large vertical gaps. Height 320 → **340** (content sits at ~315, ~25px slack — information-rich, not stretched).
- **Layout ratio** 2fr/1fr → **2.2fr/1fr** (~69/31); gap 14→16.
- **Right rail visibility** ~4.5 → **~3.3 cards** (cards ≈94px in a 340px rail → 3 full + a partial 4th + scroll cue), matching the "3.5 visible / partial 4th" intent.
- **Relevance maturity system** — `rel` values rescaled to increase per state (top-2 cards match the PDF exactly): Day 0 Profile **74/68** · Activity **88/82** · Day 1 **94/91** · Day 10 **98/96**; remaining cards descend within each level.
- Drawer unchanged (per spec). Verified: all 4 states + sub-state, candidate re-target, drawer open/close + action confirm, no console errors; screenshots match the refinement doc.

**Featured-card left image panel (`left card revamp.pdf`):** reduce the text-heavy feel — left side only; rail/content/metrics/CTA/impact untouched. Featured card became a **horizontal split: 32% image panel + 68% content**. Card height unchanged (306). The image panel uses the dark navy→teal gradient + a **generated per-category SVG scientific motif** (`feedArt(cat)`: Technology = microfluidic channels + droplets, Product = concentric formulation rings + particles, Market = trend line + nodes, Distribution = node network + dashed route) — on-brand, no stock photos/people (per the PDF avoid-list). Overlays: **category badge top-left** (moved off the content top row, white-on-translucent + category dot), **`NN% Match` bottom-left**, and a 30%→5%→40% dark gradient for badge readability; image corners follow the card radius via `overflow:hidden`. Content moved unchanged into `.opps-feed-main`; top row forced single-line (`flex-wrap:nowrap` + `.opps-src` ellipsis + `.opps-live` no-shrink) so the footer still fits 306. Motif/badge/match swap with the selected candidate. No hover-animation change. Verified across states: select re-target, star/dismiss, drawer all intact, no console errors.

**Match-to-reference pass (`left card revamp` follow-up screenshot):** matched the card to the supplied reference visual + image type. Image panel now uses a **real photographic** `background-image` per category (`PHOTO` map → Unsplash: Technology circuit macro, Product lab vials, Market chart, Distribution logistics — same proven `background-image` pattern as `.sc-bg`, so it loads in the real app; the generated `feedArt` SVG stays behind as a graceful fallback if a photo is blocked/slow). Image **category badge** gained the compass icon. **Match overlay → glass chip**: `NN%` + teal SVG progress ring (`matchRing`) + "Match to your portfolio" on a translucent blurred panel, bottom-left. **Star/dismiss → outlined** (white, 1px border, 30px, 9px radius). **Metrics → no gray box**: top hairline + vertical column dividers, bold 15px values. **LIVE → borderless** dot+label; subtitle bumped to 13px/500. Top row kept single-line (source · LIVE); the inline category pill from the reference was dropped (kept only on the image) because the real card's content panel is ~295px — far narrower than the wide reference banner — so pill+full source+LIVE can't coexist without truncating the source.

**Final visual refinements (`recommendation-final-refinements.md`):** calmer, more enterprise, integrated with the dashboard. Interactions / drawer / states / IA preserved.
- **Featured card → neutral surface.** Dropped the dark-green gradient (`#04332f→#021a24`) and the always-on teal radial; now `var(--white)` + `var(--border)` + soft shadow, like the signal cards. All inner text recolored dark-on-light (title `--text-1`, subt `--text-2`, desc `--text-3`; metrics box → `--page-bg` + `--border`; LIVE pill + Impact badge + Explore CTA → teal-on-light; feed category pill switched from the `c.d` dark-bg variant to `c.l`+`c.bg`). **Hover = subtle teal radial highlight** (`::after` opacity 0→1) + soft shadow lift only — "intelligence panel," not a second hero.
- **Tighter height.** Rail + featured 340 → **306px**; right rail now shows **exactly 3 full cards + an ~11% peek** of the 4th (signal cards trimmed to ≈89px: padding 10→8, title line-height 1.28→1.24). Featured internal gaps reduced (header→title 20→16, desc→metrics 24→16, metrics→footer 20→14) to pull metrics/CTA up; bottom whitespace inside the card down from ~25px to ~11px.
